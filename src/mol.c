...
int get_next_atom(mol *mol)
{
  ...
  if (mol->work.work_ptr == mol->info.cursor.pos.pos)
...
#ifndef _WIN32
static double __max(double a, double b)
{
  return a >= b ? a : b;
}
#endif
...
int DrawCycle(mol *mol, double prev_theta)
{
  ...
  if (mol->work.work_ptr == mol->info.cursor.branch.pos && mol->info.cursor.mode == MODE_BR_SELECT)
    mol->info.color = GRAY;
  ...
  while (i < n)
  {
    int r = DrawMolStep(mol, -theta, mol->work.work_bond.angle, theta);
    if (r < 0)
      return -1;
    if (r == 1)
    {
...
int DrawMol_inside(mol *mol)
{
  ...
  if (DrawAtom(mol))
    return -1;
  int ret;
  while ((ret = DrawMolSpecialFeatures(mol, 0, NaN)) != RET_OVER)
  {
    if (ret <= RET_ERROR)
      return ret;
    if (ret == RET_END_OF_BRANCH)
      return 0;
  }
...
int DrawMolStep(mol *mol, double branch_angle_offset, double bond_default_angle, double prev_theta)
{
  ...
  int ret;
  while ((ret = DrawMolSpecialFeatures(mol, branch_angle_offset, prev_theta)) != RET_OVER)
    if (ret)
      return ret;
...
int create_mol(mol *empty_mol, char *text)
{
  ...
  *empty_mol = (mol){
      .info = (mol_info){
          ...
          .cursor.pos.pos = text + 1,
      },
  };

  skip_white_space(&empty_mol->info.text);

  if (empty_mol->info.cursor.pos.pos[0] == '[')
    if (skip_next_angle(&empty_mol->info.cursor.pos.pos))
      return -1;
